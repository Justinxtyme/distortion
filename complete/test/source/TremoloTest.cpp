...
/** This test extracts the LFO used by the Fuzz effect and saves it to a WAV
 * file "lfo.wav".
 *
 * You can find the file in the same folder where the test executable resides,
 * most probably [CMake binary dir]/test/.
 *
 * The LFO rate is not changed; thus, the one set in Fuzz's constructor will
 * be used.
 */
...
/** This test extracts the LFO used by the Fuzz effect switching the LFO
...
TEST(Tremolo, SamplewiseAndChannelwiseProcessingYieldIdenticalResults) {
  ...
  // create 2 identical Fuzz instances with default parameters
